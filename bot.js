...
// ── Олон Gemini key — таслалаар тусгаарлаж өгнө: "key1,key2,key3" ──
...
// Key бүрд нэг model instance урьдчилан үүсгэнэ
const models = GEMINI_KEYS.map(key => {
  const genAI = new GoogleGenerativeAI(key);
  return genAI.getGenerativeModel({ model: MODEL, systemInstruction: SYSTEM_PROMPT });
});

if (models.length === 0) {
  console.error("⚠️ GEMINI_API_KEYS тохируулаагүй байна! Дор хаяж нэг key хэрэгтэй.");
}

let currentKey = 0; // одоо ашиглаж буй key-ийн индекс

// ── Cooldown — хэрэглэгч бүрийн сүүлийн хүсэлтийн цаг ──
const lastRequest = new Map(); // userId -> timestamp
...
function checkCooldown(userId) {
  ...
  if (remaining > 0) {
    return Math.ceil(remaining / 1000); // үлдсэн секунд
  }
  lastRequest.set(userId, now);
  return 0; // cooldown дууссан, зөвшөөрнө
}
...
async function askAI(message, userName) {
  if (models.length === 0) throw new Error("Gemini key байхгүй");

  let lastError;
  // Бүх key-г нэг бүрчлэн оролдоно
  for (let attempt = 0; attempt < models.length; attempt++) {
    const idx = (currentKey + attempt) % models.length;
    try {
      const result = await models[idx].generateContent(
        `[Тоглогч: ${userName}] ${message}`
      );
      currentKey = idx; // амжилттай key-г санана
      ...
      // Өөр төрлийн алдаа бол шууд зогсооно
      throw e;
    }
  }
  // Бүх key дүүрсэн
...
client.once("ready", async () => {
  console.log(`Bot бэлэн: ${client.user.tag} (${models.length} Gemini key)`);
  await setupPanel();
});
